...
class M1SPlayer extends Player {
  ...
  async readTargetPlaybackState() {
    ...
    const memberQueueDepth = attrs.member_queue_depth_ms && typeof attrs.member_queue_depth_ms === 'object'
      ? attrs.member_queue_depth_ms
      : {};
    let maxMemberQueueSeconds = 0;
    for (const value of Object.values(memberQueueDepth)) {
      const milliseconds = Number(value);
      if (Number.isFinite(milliseconds) && milliseconds > 0) {
        maxMemberQueueSeconds = Math.max(maxMemberQueueSeconds, milliseconds / 1000);
      }
    }
    return {
      ...
      while (!token.cancelled && generation === this.playGeneration) {
        try {
          ...
          if (isOurSource && active) {
            ...
            // Finite-media EOF makes the HA group entity IDLE after the shared
            // source queue has been exhausted, but PCM already fanned out can still
            // be waiting in each member queue and in the hub/ALSA buffer. Drain both
            // layers before asking YTM for the next item. This uses integration
            // transport diagnostics only; no song-duration/timeline guess is used.
            const hubTailSeconds = Math.max(
              snapshot.remotePrefillSeconds || 0,
              snapshot.maxAlsaDelaySeconds || 0
            );
            const bufferedTailSeconds = (snapshot.maxMemberQueueSeconds || 0) + hubTailSeconds;
            const drainMs = Math.max(500, Math.min(9000, Math.round((bufferedTailSeconds + 0.50) * 1000)));
            log('info', `[${this.definition.name}] Real EOF drain before Next: ${(drainMs / 1000).toFixed(2)}s`,
              `member_queue=${(snapshot.maxMemberQueueSeconds || 0).toFixed(2)}s hub=${hubTailSeconds.toFixed(2)}s`);
